...
```{r ,rate-scatter, warning=FALSE, cache=TRUE,echo=F, message=FALSE, include=TRUE, eval=TRUE,fig.height=8 }
...
```{r ,rate-spread1, warning=FALSE, cache=TRUE,echo=F, message=FALSE, include=TRUE, eval=TRUE,fig.height=6 }
...
```{r ,rate-spread2, warning=FALSE, cache=TRUE,echo=F, message=FALSE, include=TRUE, eval=TRUE,fig.height=6 }
...
```{r ,rate-spread3, warning=FALSE, cache=TRUE,echo=F, message=FALSE, include=TRUE, eval=TRUE,fig.height=6 }
rdata[year>2005,sav2:=mean(mrate-rate10y,na.rm=T)]

ggplot(data=rdata[year>2005],aes(x=date,y=mrate-rate10y))+
  geom_line(color=plasma(5)[1])+
  geom_ribbon(aes(ymin=sav2,ymax=mrate-rate10y),fill=plasma(5)[2],alpha=0.5)+
  theme_minimal()+ geom_line(aes(y=sav2),color="black",linetype=2,size=1.15)+
  theme(plot.title=element_text(size=14))+
    theme(legend.justification=c(0,0), legend.position="top")+
    theme(plot.caption=element_text(hjust=0,vjust=1,margin=margin(t=10)))+
    theme(plot.margin=unit(c(0.25,0.25,0.25,0.25),"cm"))+
    labs(x="",y="Rate (%)",title="Spread: 30-year Fixed Mortgag - 10-year Treasury",
         subtitle="2011-2016",
         caption="@lenkiefer Source: Freddie Mac, Federal Reserve dotted line average spread (2005-2016)")
...
```{r ,ehs-bars, warning=FALSE, cache=TRUE,echo=TRUE, message=FALSE, include=TRUE, eval=TRUE }
...
```{r ,ehs-gif, warning=FALSE, cache=TRUE,echo=TRUE, message=FALSE, include=TRUE, eval=FALSE }
...
```{r ,nhs-band, warning=FALSE, cache=TRUE,echo=TRUE, message=FALSE, include=TRUE, eval=T }
...
```{r ,nhs-gif, warning=FALSE, cache=TRUE,echo=TRUE, message=FALSE, include=TRUE, eval=F }
...
```{r ,rate-plots_Demo, warning=FALSE, cache=TRUE,echo=T, message=FALSE, include=TRUE, eval=F,fig.height=8 }
rdata <- fread("data/rates.txt")  #load data
rdata$date<-as.Date(rdata$date, format="%m/%d/%Y")
